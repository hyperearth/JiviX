...
#define MatID geomMTs[nonuniformEXT(nodeMeshID)].materialID[geometryInstanceID]
    ...
    const float4 diffuseColor = toLinear(unit. diffuseTexture >= 0 ? texture(sampler2D(textures[nonuniformEXT(unit. diffuseTexture)],samplers[2u]), fTexcoord.xy) : unit.diffuse);
