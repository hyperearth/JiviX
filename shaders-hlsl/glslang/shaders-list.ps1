...
function BuildCompute($Name, $InDir = "", $OutDir = "", $AddArg = "", $AltName = $Name) {
    $ARGS = "$CMPPROF $InDir$Name.hlsl -o $OutDir$AltName.spv $CFLAGSV"
...
function BuildAllShaders($Pfx = "") {
    ...
    #BuildCompute "rasterize.geom"       "$INDIR$RNDX" "$HRDDIR$RTPU" "-DCONSERVATIVE" "covegeomence.gs"
    ...
    #
     BuildCompute "mapping.frag"       "$INDIR$RNDX" "$HRDDIR$RTPU"
     BuildCompute "mapping.geom"       "$INDIR$RNDX" "$HRDDIR$RTPU"
     BuildCompute "mapping.vert"       "$INDIR$RNDX" "$HRDDIR$RTPU"
