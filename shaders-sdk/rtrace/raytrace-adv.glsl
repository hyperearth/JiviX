void main() {
    const uvec2 lanQs = LAUNCH_ID;//gl_LaunchIDEXT.xy;//gl_GlobalInvocationID.xy;
    launchSize = imageSize(writeImages[IW_POSITION]);

    for (uint Q = 0u; Q < 2u; Q++) {
        const uvec2 lanQ = uvec2(lanQs.x, Q*workY + lanQs.y);
        ...
        vec4 diffused = vec4(RES.diffuseColor.xyz, 1.f), emission = vec4(RES.emissionColor.xyz, 1.f);
    ...
    // 
    {
        ...
        const uvec2 lanQ = uvec2(LAUNCH_ID.x, (LAUNCH_ID.y<<1u) | ((LAUNCH_ID.x+rdata.x)&1u));
        const uint lIdx = LAUNCH_ID.y * workX + LAUNCH_ID.x;

        XHIT GRD = hits[lIdx], RES = hits[lIdx];
        if ( (RES.diffuseColor.w > 0.001f && RES.geoNormal.w < 9999.f) ) {
            vec3 origin = hits[lIdx].origin.xyz;
            vec3 raydir = hits[lIdx].direct.xyz;
            ...
            for (uint I=0;I<MAX_ITERATION;I++) {
...

                {
                    ...
                    for (uint i=0;i<2;i++) { // fast trace
                        XHIT result = traceRays(origin, raydir, normal, 10000.f, true, 0.01f); // Useless to tracing for transparency...
                        if ( i == 0 && I == 2 ) { GRD = result; };
