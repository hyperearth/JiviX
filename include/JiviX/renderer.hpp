...
namespace jvx { 
    class Renderer : public Wrap<jvi::Renderer> { public: using T = jvi::Renderer;
        ...
        Renderer(const vkt::uni_arg<jvx::Context>& context) { object = std::make_shared<jvi::Renderer>(*context); };
        Renderer(const jvx::Context& context) { object = std::make_shared<jvi::Renderer>(context); };
