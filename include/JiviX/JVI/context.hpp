...
#pragma pack(pop)
    ...
    class Context : public std::enable_shared_from_this<Context> { public: friend MeshInput; friend MeshBinding; friend Node; friend Driver; friend Material; friend Renderer;
        ...
        protected: virtual uPTR(Context) construct() {//
            ...
            this->uniformGPUData = vkt::Vector<jvi::Matrices>(std::make_shared<vkt::VmaBufferAllocation>(this->driver->getAllocator(), vkh::VkBufferCreateInfo{ .size = VkDeviceSize(sizeof(Matrices) * 2u), .usage = hostUsage }, vkt::VmaMemoryInfo{ .memUsage = VMA_MEMORY_USAGE_GPU_ONLY }));
            this->uniformRawData = vkt::Vector<jvi::Matrices>(std::make_shared<vkt::VmaBufferAllocation>(this->driver->getAllocator(), vkh::VkBufferCreateInfo{ .size = VkDeviceSize(sizeof(Matrices) * 2u), .usage = gpuUsage }, vkt::VmaMemoryInfo{ .memUsage = VMA_MEMORY_USAGE_CPU_TO_GPU }));
        ...
        protected: virtual uPTR(Context) createDescriptorSetLayouts() { // reset layout descriptions
            if (!this->unifiedPipelineLayout) {
                ...
                //
                this->materialDescriptorSetLayoutHelper.pushBinding(vkh::VkDescriptorSetLayoutBinding{ .binding = 20u, .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, .descriptorCount = 1u, .stageFlags = pipusage }, indexedf);
        ...
        protected: virtual uPTR(Context) createDescriptorSets() {
            ...
            { // For Reprojection Pipeline
                ...
                { //
                    vkh::VsDescriptorHandle<vkh::VkDescriptorImageInfo> handle = descInfo.pushDescription(vkh::VkDescriptorUpdateTemplateEntry{ .dstBinding = 16u, .descriptorCount = 12u, .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE });
                    for (uint32_t i = 0; i < 12u; i++) { handle.offset<vkh::VkDescriptorImageInfo>(i) = vkt::ImageRegion(frameBfImages[i]).getDescriptor(); };
                }

                {
                    vkh::VsDescriptorHandle<vkh::VkDescriptorImageInfo> handle = descInfo.pushDescription(vkh::VkDescriptorUpdateTemplateEntry{ .dstBinding = 17u, .descriptorCount = 12u, .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE });
                    for (uint32_t i = 0; i < 12u; i++) { handle.offset<vkh::VkDescriptorImageInfo>(i) = smFlip0Images[i].getDescriptor(); };
                };

                {
                    vkh::VsDescriptorHandle<vkh::VkDescriptorImageInfo> handle = descInfo.pushDescription(vkh::VkDescriptorUpdateTemplateEntry{ .dstBinding = 18u, .descriptorCount = 12u, .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE });
                    for (uint32_t i = 0; i < 12u; i++) { handle.offset<vkh::VkDescriptorImageInfo>(i) = smFlip1Images[i].getDescriptor(); };
                };
