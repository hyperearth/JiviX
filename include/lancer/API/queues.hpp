...
namespace lancer {
    ...
    class Task_T : public std::enable_shared_from_this<Task_T> {
        ...
        public: 
            Task_T(const QueueHelper& queue, const std::function<std::future<api::Result>(api::CommandBuffer&, const uintptr_t&)>& caller = {}, api::CommandBuffer* cmdbuf = nullptr): _caller(caller), _queue(queue), _cmdbuf(cmdbuf) {
